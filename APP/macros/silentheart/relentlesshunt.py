...
class relentlessHuntListener:  
    ...
    def stack(self, keybind, flowstatekeybind):
        ...
        def relentlessHunt(*_): #ignores all arguments passed to it 
            ...
            win32api.mouse_event(win32con.MOUSEEVENTF_RIGHTUP if is_mouse_swapped() else win32con.MOUSEEVENTF_LEFTUP, 0, 0, 0, 0)
            time.sleep(0.03)
            keyboard.press(flowstatekeybind)
            time.sleep(0.01)
            keyboard.release(flowstatekeybind)
            win32api.mouse_event(win32con.MOUSEEVENTF_RIGHTDOWN if is_mouse_swapped() else win32con.MOUSEEVENTF_LEFTDOWN, 0, 0, 0, 0)
            time.sleep(0.01)
            win32api.mouse_event(win32con.MOUSEEVENTF_RIGHTUP if is_mouse_swapped() else win32con.MOUSEEVENTF_LEFTUP, 0, 0, 0, 0)
            time.sleep(0.01)
            ...
            win32api.mouse_event(win32con.MOUSEEVENTF_RIGHTUP if is_mouse_swapped() else win32con.MOUSEEVENTF_LEFTUP, 0, 0, 0, 0)
            time.sleep(0.28)
        self.hotkey = keyboard.on_press_key(keybind, relentlessHunt)
